...
Use const when you need to declare a variable that will not be reassigned. This includes constant values, such as mathematical constants or enumerated values.
...
var variables do not need to be initialized when they are declared. If a var variable is not initialized, its value will be undefined.
...
let and const variables are hoisted to the top of their block scope, but they are not initialized. This means that using a let or const variable before it is initialized will throw a ReferenceError.
When to use var, let, and const

It is generally recommended to use let or const instead of var. This is because let and const variables have block scope, which makes the code more predictable and easier to debug.
...
*The temporal dead zone (TDZ) is a period of time during which a variable declared with let is not yet initialized. The TDZ begins at the start of the block in which the variable is declared and ends when the variable is initialized.

During the TDZ, accessing the variable will throw a ReferenceError. This is because the variable does not yet exist. However, hoisting still applies to let variables, so they can be accessed before they are declared, but only after the TDZ has ended.
...
console.log(x); // 10
The TDZ is a safety feature that helps to prevent errors. It ensures that variables are always initialized before they are used. This makes the code more predictable and easier to debug.
*/
